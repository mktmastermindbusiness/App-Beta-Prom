...
const NOTIFICATIONS_CONFIG = {
    ...
    // === BALANÇO GERAL DE CAIXAS ===
    balancoCaixas: {
        enabled: true,
        titulo: "Balanço Geral de Caixas",
        schedule: {
            type: 'weekly',
            days: [1],
            startTime: "06:00",
...
(function () {
    ...
    function gerenciarQuadrosOriginais() {
            ...
            if (NOTIFICATIONS_CONFIG.estoque.enabled) {
                ...
                item.innerHTML = `
                    <div class="notification-content">
                        ...
                        <button class="btn-preencher" onclick="irPreencher('inicial')"><i class="fas fa-edit"></i> Preencher</button>
                    </div>
                `;
            ...
            if (NOTIFICATIONS_CONFIG.estoqueF.enabled) {
                ...
                itemF.innerHTML = `
                    <div class="notification-content">
                        ...
                        <button class="btn-preencher" onclick="irPreencher('final')"><i class="fas fa-edit"></i> Preencher</button>
                    </div>
                `;
                board.appendChild(itemF);
            }

            // Card — Balanço Geral de Caixas
            if (NOTIFICATIONS_CONFIG.balancoCaixas.enabled) {
                const itemB = document.createElement('div');
                itemB.id = 'item-balanco-caixas';
                itemB.className = 'notification-item';
                itemB.innerHTML = `
                    <div class="notification-content">
                        <div class="notification-label">${NOTIFICATIONS_CONFIG.balancoCaixas.titulo}</div>
                        <div id="dia-rotulo-b" class="dia-rotulo">---</div>
                        <div id="timer-label-b" class="timer-label">---</div>
                        <div id="contagem-regressiva-b" class="timer">00:00:00</div>
                        <button class="btn-preencher" onclick="irPreencher('caixas')"><i class="fas fa-edit"></i> Preencher</button>
                    </div>
                `;
                board.appendChild(itemB);
            }
        ...
        const algumAtivo = NOTIFICATIONS_CONFIG.estoque.enabled || NOTIFICATIONS_CONFIG.estoqueF.enabled || NOTIFICATIONS_CONFIG.balancoCaixas.enabled;
    ...
    function atualizarTodosOsTimers() {
        ...
        if (NOTIFICATIONS_CONFIG.balancoCaixas.enabled) {
            atualizarBoard('balancoCaixas', 'dia-rotulo-b', 'contagem-regressiva-b', 'item-balanco-caixas', 'timer-label-b');
        }
...
/* ── Navegação para preenchimento ──────────────────────────── */
window.irPreencher = function (tipo) {
    window.location.href = 'estoque.html#' + tipo;
};
