(function () {
  ...
  window.GlobalState = {
    ...
    promptIfNeeded: function (opts) {
      opts = opts || {};
      if (this.isLoggedIn() || this.getSelection()) return Promise.resolve();
      var self = this;
      return this.loadConfig().then(function (cfg) {
        ...
        return self._showRedeLojaModal();
      });
    },
    ...
    _updateBadge: function () {
      ...
      var html = '<span class="gs-badge" style="display:inline-flex;align-items:center;gap:6px;background:#1e1b4b;color:white;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;white-space:nowrap;">';
      if (info) html += '🏪 ' + info.rede + ' › ' + info.loja;
      if (session) html += ' 👤 ' + session.email;
      html += ' <button onclick="GlobalState.logout()" style="background:rgba(255,255,255,0.2);border:none;color:white;border-radius:50%;width:20px;height:20px;font-size:12px;cursor:pointer;line-height:1;padding:0;">✕</button>';
      html += '</span>';
    ...
    _showRedeLojaModal: function () {
      ...
      this._createModal(
        ...
        '<button id="gs-confirm" disabled style="width:100%;padding:14px;background:linear-gradient(135deg,#1e1b4b,#4f46e5);color:white;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;opacity:0.5;">Confirmar</button>'
      );
    ...
    _showLojaStep: function (user) {
      ...
      if (lojasPermitidas.length === 1) {
        ...
        this.setSelection(user.rede, user.lojaAtual, user.promotor, user.estado);
        return;
      }
      ...
      this._createModal(
        ...
        '<button id="gs-loja-confirm" style="width:100%;padding:14px;background:linear-gradient(135deg,#1e1b4b,#4f46e5);color:white;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;">Confirmar</button>'
      );
